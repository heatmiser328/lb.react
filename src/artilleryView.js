            ...
            <View style={{flex: 1}}>
                ...
                <View style={{flex:1, flexDirection:'row'}}>
                    <View style={{flex:3, justifyContent: 'flex-start'}}>
                        <SelectList title={'Artillery'} titleonly={true}
                            items={this.artillery().map((a) => {return {label: a, value: a};})}
                            selected={this.state.artillery}
                            onChanged={this.onArtilleryChanged}/>
                    </View>
                    <View style={{flex:2}}>
                        <MultiSelectList title={'Modifiers'}
                            items={this.modifiers().map((m) => {return {name: m.name, selected: this.state.mods[m.name]};})}
